...
// gorev: Planım'da tamamlanan plan görevi (Pomodoro'dan ayrı sayaç → çift-say yok)
export const XP_PER = { konu: 5, deneme: 20, cozulen: 0.2, yanlis: 5, dakika: 1, kullanim: 1, soru: 10, gorev: 3 }
...
export const ROZETLER = [
  { id: 'ilk-adim', ad: 'İlk Adım', ikon: '👣', aciklama: 'İlk çalışmanı yaptın', kosul: (s) => (s.konuDone + s.denemeSayisi + s.soruCozulen + s.yanlisSayisi + (s.planGorev || 0)) > 0 },
  ...
  { id: 'planci', ad: 'Planlı', ikon: '🗓️', aciklama: '25 plan görevi bitirdin', kosul: (s) => (s.planGorev || 0) >= 25 },
...
export function guestStats() {
  ...
  // Planım: tüm hafta anahtarlarındaki tamamlanmış görevleri say (unisense_plan_YYYY-Www)
  let planGorev = 0
  try {
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i)
      if (k && /^unisense_plan_\d{4}-W\d{2}$/.test(k)) {
        for (const t of j(k, '[]')) if (t && t.status === 'done') planGorev++
      }
    }
  } catch { /* noop */ }
  ...
  return {
    konuDone, denemeSayisi, soruCozulen, planGorev,
