...
class TestMerkeziIstek:
    def test_gecici_hatada_yeniden_dener(self, monkeypatch):
        import requests
        from unisense.infrastructure.scrapers import kariyer_scraper as ks
        ...
        r = ks._istek(SahteOturum(), "GET", "http://x")
        assert r.status_code == 200
        assert deneme["n"] == 3

    def test_404te_birakir(self):
        import pytest
        import requests
        from unisense.infrastructure.scrapers import kariyer_scraper as ks
        ...
        class Sahte404:
            ...
            def raise_for_status(self):
                raise requests.HTTPError("yok", response=self)
        ...
        with pytest.raises(requests.HTTPError):
            ks._istek(oturum, "GET", "http://x")
        assert oturum.n == 1  # tek deneme, bırakır

    def test_500de_uc_deneme(self, monkeypatch):
        import requests
        from unisense.infrastructure.scrapers import kariyer_scraper as ks
        monkeypatch.setattr(ks.time, "sleep", lambda s: None)
        ...
        class Sahte500:
            ...
            def raise_for_status(self):
                raise requests.HTTPError("bozuk", response=self)
